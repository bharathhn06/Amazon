...
public class Clarification {
    public static void main(String[] args) throws InterruptedException, IOException, AWTException {
     ...
     // Summary section
        ...
        tableHtml.append("<p style='font-weight: bold;'>Total Clarifications created today : ");
        ...
        tableHtml.append("<tr style='background-color: #0070C0;'><th style='text-align: center; font-weight: bold;'>SL.NO</th><th style='text-align: center; font-weight: bold;'>Assignee</th><th style='text-align: center; font-weight: bold;'>Count</th></tr>");
        ...
        for (Map.Entry<String, Integer> entry : assigneeCount.entrySet()) {
            tableHtml.append("<tr><td style='text-align: center;'>").append(serialNumber).append("</td><td style='text-align: center;'>").append(entry.getKey()).append("</td><td style='text-align: center;'>").append(entry.getValue()).append("</td></tr>");
        ...
        tableHtml.append("<table border='1' style='text-align: center;'>");
        tableHtml.append("<tr style='background-color: #0070C0;'><th style='text-align: center; font-weight: bold;'>Issue ID</th><th style='text-align: center; font-weight: bold;'>Submitter Identity</th><th style='text-align: center; font-weight: bold;'>Assignee Identity</th></tr>");
        for (Row row : rowList) {
            if (row != null) {
                ...
                if (labelsCell == null || labelsCell.getStringCellValue() == null || labelsCell.getStringCellValue().isEmpty()) {
                    if (dateCell != null && dateCell.getCellType() == CellType.NUMERIC) {
                        ...
                        if (!date.isEqual(today) && !date.isEqual(today.minusDays(1))) {
                            ...
                            Cell submitterCell = row.getCell(7); 
                            Cell assigneeCell = row.getCell(8); 
                            if (issueIdCell != null && submitterCell != null && assigneeCell != null) {
                                ...
                                if (rowCounter == 0 && issueIdValue.contains("/issues/")) { // Convert 2nd column to link format starting from the 2nd row
                                    ...
                                    System.out.println(issueIdValue);
                                }
                                tableHtml.append("<tr><td style='text-align: center;'>").append(issueIdValue).append("</td><td style='text-align: center;'>").append(submitterCell.getStringCellValue()).append("</td><td style='text-align: center;'>").append(assigneeCell.getStringCellValue()).append("</td></tr>");
                            }
                        }
                    } else {
                        System.out.println("Date cell is not numeric");
            ...
            if (row != null) {
                ...
                if (printRow) {
                    if (rowCounter == 0) { // Check if it's the first row
                        tableHtml.append("<tr style='background-color: #0070C0;'>"); // Set the background color to the specified color
                        tableHtml.append("<td style='text-align: center; border: 2px solid black; padding: 5px; background-color: #0070C0; font-weight: bold;'>SL.NO</td>"); // Add "SL.NO" to the first cell
                    } else {
                        tableHtml.append("<tr>");
                        tableHtml.append("<td style='text-align: center; border: 2px solid black; padding: 5px; background-color: #0070C0;'>").append(serialNumber).append("</td>"); // Add serial number cell
                    }
                    for (int j = 0; j < row.getLastCellNum(); j++) {
                        if (j == 3 || j == 4 || j == 6) { // Skip 5th and 7th columns (index 4 and 6)
                            continue;
                        ...
                        if (cell != null) {
                            ...
                            if (rowCounter == 0) { // Check if it's the first row
                                if (j == 1) { // Set the 2nd column header
                                    tableHtml.append("<td style='text-align: center; border: 2px solid black; padding: 5px; background-color: #0070C0; font-weight: bold;'>Link</td>"); 
                                } else {
                                    tableHtml.append("<td style='text-align: center; border: 2px solid black; padding: 5px; background-color: #0070C0; font-weight: bold;'>").append(cellValue).append("</td>"); // Make the text bold and fill the cell with the specified color
                                }
                            } else {
                            ...
                            if (rowCounter == 0) { // Check if it's the first row
                                tableHtml.append("<td style='text-align: center; border: 2px solid black; padding: 5px; background-color: #0070C0; font-weight: bold;'>&nbsp;</td>"); // Fill the cell with the specified color and make the text bold
                            } else {
