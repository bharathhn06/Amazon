...
public class Audit_close {

	public static void main(String[] args) throws InterruptedException, IOException, AWTException {
		WebDriver driver = new FirefoxDriver();
		driver.manage().window().maximize();
					driver.get("https://issues.amazon.com/issues/search?q=containingFolder%3A(2a42d4ee-18c1-4cf8-8a1e-b1344ff622c8)+createDate%3A(%5BNOW-2MONTHS..NOW%5D)&sort=lastUpdatedDate+desc&selectedDocument=4bcf3a6b-892f-43e6-a53b-5f19007c3ddb");	
		...
		driver.findElements(By.xpath("//*[@placeholder='Add a subject']")).get(0).sendKeys("Audit summary for Execution and Non-Execution 30 Days");
			...
			if (row != null) {
				...
				if (createDateCell != null) {
					if (createDateCell.getCellType() == CellType.NUMERIC) {
						...
						if (createDate.getYear() == lastMonth.getYear() && createDate.getMonthValue() == lastMonth.getMonthValue() ) { //&& createDate.getMonthValue() == lastMonth.getMonthValue()
							AuditCount++;
						}
					}
				}
		...
		tableHtml.append("<p style='font-weight: bold;'>Total No.of Audit in the month of " + lastMonth.getMonth().toString()  + "-" + lastMonth.getYear() + " : " + AuditCount + "</p>");
		...
		for (Row row : rowList) {
			if (row != null) {
				...
				if (createDateCell != null) {
					if (createDateCell.getCellType() == CellType.NUMERIC) {
						LocalDate createDate = LocalDate.ofEpochDay((long) (createDateCell.getNumericCellValue() - 25569));
						if (createDate.getYear() == lastMonth.getYear() && createDate.getMonthValue() == lastMonth.getMonthValue() ){ { // Start from the second row (index 1)
							if (row != null) {
								Cell thirdColumnCell = row.getCell(3); // 4th column (index 3)
								Cell fourthColumnCell = row.getCell(4); // 5th column (index 4)
		...
		tableHtml.append("<p style='font-weight: bold;'>Audits raised in the month of " + lastMonth.getMonth().toString() + " - " + lastMonth.getYear() + " : " + "</p>");
		...
		for (Row row : rowList) {
		    if (row != null) {
		        if (rowCounter == 0) { // Print the first row as it is
		            ...
		            if (createDateCell != null) {
		                LocalDate createDate = LocalDate.ofEpochDay((long) (createDateCell.getNumericCellValue() - 25569));
		                if (createDate.getYear() == lastMonth.getYear() && createDate.getMonth() == lastMonth.getMonth()) {
